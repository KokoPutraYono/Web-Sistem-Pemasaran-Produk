...
<body>
	<ul>
	  ...
	  <li style="float: right; margin-right: 50px;">
		...
		<!-- moon atau bulan -->
		<!-- <a href="#" class="bi bi-moon-stars-fill text-warning" ></a> -->
      </li>
    @endauth
    @guest
      <li><a href="/dassboard">Dashboard</a></li>
	...
	@foreach($logo as $lo)
	<form action="/admin/settingUpdate/{{$lo->id}}" method="post" style="padding-top: 10%">
		{{ csrf_field() }}
		<div class="wrapper">
			<label class="font">Nama Web</label>
			<input type="text" name="logo" style="background-color: white" value="{{$lo->nama_web}}">
			<div class="popup" onclick="myFunction()"><button style="background-color: #2874A6; color: white; width: 140%; height: 100%;">Update</button>
				<span class="popuptext" id="myPopup">Update</span>
			</div>
		</div>
	</form>
	@endforeach

...
<body>
	...
	@foreach($user_edit as $e)
	<form action="/admin/userUpdate/{{$e->id}}" method="post">
		{{ csrf_field() }}
		<div class="wrapper">
			<label for="username" class="font">Nama User</label>
			<input type="text" name="username" style="background-color: white" value="{{ $e->name }}" placeholder="">
			<label for="email" class="font">E-Mail</label>
			<input type="text" name="email" style="background-color: white" value="{{ $e->email }}" placeholder="">
			<label for="pass" class="font">Password</label>
			<input type="text" readonly="readonly" style="background-color: white" name="pass" value="{{ $e->password }}" placeholder="">
			<div class="popup" onclick="myFunction()">
				<button style="background-color: #2874A6; color: white; width: 100%; height: 100%;">Update</button>
				<span class="popuptext" id="myPopup">Data diupdate</span>
			</div>
		</div>
	@endforeach
	</form>

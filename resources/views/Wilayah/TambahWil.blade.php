...
<body>
	...
	@foreach($wilayah_tambah as $w)
	<form action="/admin/wilayahStore" method="post">
		{{ csrf_field() }}
		<div class="wrapper">
			<label for="kodeproduk" class="font">Kode Produk</label>
			<input type="text" readonly="readonly" name="kodeproduk" style="background-color: white" id="" placeholder="" value="{{$w->kode_produk}}">
			<label for="namaproduk" class="font">Nama Produk</label>
			<input type="text" readonly="readonly" name="namaproduk" style="background-color: white" id="" placeholder="" value="{{$w->nama_produk}}">
			<label for="wilayah" class="font">Wilayah</label>
			<textarea name="wilayah" placeholder="" rows="5" style="background-color: white" cols="10"></textarea>
			<div class="popup" onclick="myFunction()"><button style="background-color: #2874A6; color: white; width: 140%; height: 100%;">Tambah</button>
				<span class="popuptext" id="myPopup">Wilayah ditambahkan</span>
			</div>
		</div>
	</form>
	@endforeach

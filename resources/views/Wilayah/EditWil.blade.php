...
<body>
	...
	@foreach($wilayah_edit as $e)
	<form action="/admin/wilayahUpdate/{{$e->id}}" method="post">
		{{ csrf_field() }}
		<div class="wrapper">
			<label for="kodeproduk" class="font">Kode Produk</label>
			<input type="text" readonly="readonly" name="kodeproduk" style="background-color: white" id="" placeholder="" value="{{$e->kode_produk}}">
			<label for="namaproduk" class="font">Nama Produk</label>
			<input type="text" readonly="readonly" name="namaproduk" style="background-color: white" id="" placeholder="" value="{{$e->nama_produk}}">
			<label for="wilayah" class="font">Wilayah</label>
			<textarea name="wilayah" placeholder="" rows="5" style="background-color: white" cols="10">{{$e->wilayah}}</textarea>
			<div class="popup" onclick="myFunction()">
				<button style="background-color: #2874A6; color: white; width: 100%; height: 100%;">Update</button>
				<span class="popuptext" id="myPopup">Wilayah diupdate</span>
			</div>
		</div>
	</form>
	@endforeach

...
<body>
	...
	@foreach($produk_edit as $pro)
	<form action="/admin/produkUpdate/{{$pro->id}}" method="post" enctype="multipart/form-data">
		{{ csrf_field() }}
		<div class="wrapper">
			<label for="kodebarang" class="font">Kode Barang</label>
			<input type="text" name="kodebarang" readonly="readonly" style="background-color: white" value="{{ $pro->kode_produk }}" id="" placeholder="">
			<label for="namabarang" class="font">Nama Barang</label>
			<input type="text" name="namabarang" style="background-color: white" value="{{ $pro->nama_produk }}" id="" placeholder="">
			<label for="desk" class="font">Deskripsi</label>
			<textarea name="desk" placeholder="" style="background-color: white" rows="5" cols="10">{{ $pro->deskripsi }}</textarea>
			<label for="gambar" class="font">Gambar</label><br>
			<input type="file" name="image" style="background-color: white" accept="image/*"/><br><br>
			<div class="popup" onclick="myFunction()"><button style="background-color: #2874A6; color: white; width: 140%; height: 100%;">Update</button>
				<span class="popuptext" id="myPopup">Produk diupdate</span>
			</div>
		</div>
	</form>
	@endforeach
